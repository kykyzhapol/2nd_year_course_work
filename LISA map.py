...
Загружает результаты пространственного анализа, подготавливает данные для картографирования,
и генерирует карты кластеров для 2004 и 2024 годов.
Loads spatial analysis results, prepares data for mapping,
and generates cluster maps for 2004 and 2024.
"""
...
import re  # Регулярные выражения для очистки строк / Regular expressions for string cleaning
...
def normalize_for_merge(name):
    ...
    # Срезаем стандартные приставки/окончания субъектов РФ
    # Remove standard prefixes/suffixes of Russian subjects
    for word in ['республика', 'область', 'край', 'автономныйокруг', 'ао', 'город', 'г']:
...
# ==========================================
# 2. ЗАГРУЗКА КАРТЫ И МЭТЧИНГ ЧЕРЕЗ КОНФИГ
# 2. LOADING MAP AND MATCHING VIA CONFIG
# ==========================================
...
# Защита: если регион не смапился, даем ему оригинальное имя, чтобы пустые строки не склеились
# Protection: if region not mapped, give it original name to avoid empty string merges
gdf_map.loc[gdf_map['match_key'] == "", 'match_key'] = gdf_map['name']

# Объединяем карту и таблицу результатов расчетов по защищенному ключу 'match_key'
# Merge map and results table using protected 'match_key'
merged_gdf = gdf_map.merge(df_results, on='match_key', how='left')

# Заполняем пропуски (для регионов, по которым не было данных или нет значимости)
...
# Выводим информацию о количестве регионов каждого типа / Print info about region counts
print("=== Статистика по кластерам LISA / LISA Cluster Statistics ===")
...
def plot_and_save_lisa(year_column, year_title):
    ...
    # Создаем аккуратную легенду / Create a neat legend
    legend_patches = [
    ...
    ax.legend(
        ...
        title="Типы кластеров / Аномалий\nCluster / Anomaly Types",
        loc="lower left",
        fontsize=11,
        title_fontsize=12
    )

    # Сохраняем картинку в высоком качестве для вставки в Word/LaTeX
    # Save high-quality image for insertion into Word/LaTeX
...
if __name__ == "__main__":
    ...
    # Запускаем генерацию для обоих периодов из Excel-файла
    # Run generation for both periods from Excel file
